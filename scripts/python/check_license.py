    ...
    for filename in filenames:
        ...
        with open(file_path, 'r') as file:
            ...
            if extension in ['.cpp', '.h', '.cxx', '.hxx', '.cu']:
                ...
                if content.count(license_header_c_style.strip()) == 0:
                    print(f"No header found in file: {file_path}")
            elif extension in [ '.txt', '.sh', '.py' ]:                
                if content.count(license_header_python_style.strip()) > 1:
                    print(f"Duplicate header found in file: {file_path}")
                if content.count(license_header_python_style.strip()) == 0:
                    print(f"No header found in file: {file_path}")
