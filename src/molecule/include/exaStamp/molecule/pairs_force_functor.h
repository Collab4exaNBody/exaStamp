...
namespace exaStamp
{
  ...
  struct IntramolecularPairForceOp
  {
    ...
    const IntramolecularPairParams * const __restrict__ m_pair_params = nullptr;;
    const int * const __restrict__ m_pair_param_idx = nullptr;
    const ChemicalPair * const __restrict__ m_pair_list = nullptr;
    ...
    const bool m_per_atom_charge = false;
...
//      if constexpr ( LOCK ) { cp_locks.m_locks = m_particle_locks; }
      ...
      if( param_idx >= 0 )
      {    
        const uint64_t atom_to_decode_a = m_pair_list[i][0]; //atom_from_idmap( bonds_list[i][0] ,id_map, id_map_ghosts ); 
        const uint64_t atom_to_decode_b = m_pair_list[i][1]; //atom_from_idmap( bonds_list[i][1] ,id_map, id_map_ghosts ); 
        ...
        double charge_a=0.0, charge_b=0.0;
        if( m_per_atom_charge )
        {
          charge_a = m_cells[cell_a][m_charge_field][p_a];
          charge_b = m_cells[cell_b][m_charge_field][p_b];
        }
        else
        {
          charge_a = m_species[type_a].m_charge;
          charge_b = m_species[type_b].m_charge;
        }

        double norm_r = norm(r);
        assert(norm_r>0);

        const auto& pairpot = m_pair_params[param_idx];
        auto [ de , e ] = pairpot.m_param.compute_force_energy( norm_r, charge_a * charge_b, pairpot.m_pair_weight, pairpot.m_rf_weight );
        de /= norm_r;
        const Vec3d F = de * r;
...
namespace onika
{
  namespace parallel
  {
    template<class CellsT, class ChargeFieldT, class VirialFieldT, bool vir> struct ParallelForFunctorTraits< exaStamp::IntramolecularPairForceOp<CellsT,ChargeFieldT,VirialFieldT,vir> >
    {
