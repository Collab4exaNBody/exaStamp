...
namespace YAML
{
  ...
  template<> struct convert<TorsionsPotentialParameters>
  {
    static bool decode(const Node& node, TorsionsPotentialParameters& bs)
    {
      if( !node.IsSequence() )
        {
          lerr << "TorsionPotential type is not readable from config file." << std::endl;
          abort();
        }
  ...
  template<> struct convert<TorsionPotential>
  {
    static bool decode(const Node& node, TorsionPotential& b)
    {
      ...
      if(!node["potential"])
        {
          lerr << "No potential type given in the torsion interaction. Choice are harm_torsion, torsion, cos_two, half_torsion, opls_torsion, torsion_gaff." << std::endl;
          abort();
        }
      ...
      if( !node["types"] )
        {
          lerr << "No types elements given in the torsion interaction (example types: [H, C, C, H])." << std::endl;
          abort();
        }
      ...
      if( !node["parameters"] && b.type!="no_potential" )
        {
          lerr << "No parameters elements given in the torsion interaction." << std::endl;
          abort();
        }
...
*/
        ...
        {
          lerr << "Type of torsion potential " << b.type << " unknown. Type can be harm_torsion, compass_torsion, 0.5compass_torsion, opls_torsion, cos_two, no_potential" << std::endl;
          abort();
        }
      return true;
    }
