...
namespace exaStamp
{
  ...
  struct TorsionForceOp
  {
    ...
    const ChemicalTorsion * const __restrict__ m_torsion_list = nullptr;
    ...
    ONIKA_HOST_DEVICE_FUNC inline void operator () ( size_t i ) const
    {
      ...
      const int param_idx = m_torsion_param_idx[i];
      if( param_idx >= 0 )
      {
        const uint64_t atom_to_decode_a = m_torsion_list[i][0]; // atom_from_idmap(id1,id_map,id_map_ghosts); 
        const uint64_t atom_to_decode_b = m_torsion_list[i][1]; // atom_from_idmap(id2,id_map,id_map_ghosts); 
        const uint64_t atom_to_decode_c = m_torsion_list[i][2]; // atom_from_idmap(id3,id_map,id_map_ghosts); 
        const uint64_t atom_to_decode_d = m_torsion_list[i][3]; // atom_from_idmap(id4,id_map,id_map_ghosts); 
        ...
        const auto force_energy = intramolecular_func( m_func_params[ param_idx ], phi ); //it->second->force_energy( phi );
...
namespace onika
{
  namespace parallel
  {
    template<class CellsT, class VirialFieldT, bool vir> struct ParallelForFunctorTraits< exaStamp::TorsionForceOp<CellsT,VirialFieldT,vir> >
    {
