...
#include <exaStamp/mechanical/compute_local_numneighbors.h>
...
namespace exaStamp
{
  ...
  template< class GridT,
	    ...
	    >
  class ComputeLocalNumNeighborsOperator : public OperatorNode
  {    
  ...
  public:
    ...
    inline void execute () override final
    {
      ...
      lout << "\t- Computing per-atom number of neighbors" << std::endl;
      for(size_t c=0; c<n_cells;++c)
        {
          local_structural_data[c].numneighbors.resize( cells[c].size() );
        }

      NumNeighborsOp force_op { *rcut, local_structural_data};
  ...
  template<class GridT> using ComputeLocalNumNeighborsOperatorTmpl = ComputeLocalNumNeighborsOperator<GridT>;
  
  // === register factories ===  
  ONIKA_AUTORUN_INIT(compute_local_numneighbors)
  {
    OperatorNodeFactory::instance()->register_factory( "compute_local_numneighbors", make_grid_variant_operator< ComputeLocalNumNeighborsOperatorTmpl > );
  }
