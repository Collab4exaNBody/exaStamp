...
/*           ,class = AssertGridHasFields< GridT , field::_rx0, field::_ry0, field::_rz0 > */
  ...
  public:
    ...
    inline void execute () override final
    {
      ...
      static const std::string data_line_ref = onika::format_string(data_line_format, 1 , 1 , -1.0e103 , -1.0e103 , -1.0e103 );
      static const size_t data_line_size = data_line_ref.length();

      static const char* vel_line_format = "%12llu % .16e % .16e % .16e ";
      static const std::string vel_line_ref = onika::format_string(vel_line_format, 1 , -1.0e103 , -1.0e103 , -1.0e103 );
...
*/
      ...
      std::string domain_header = onika::format_string("# Generated by exaStamp V3\n%12llu  atoms\n%12llu  atom types\n\n%.12f\t%.12f  xlo xhi\n%.12f\t%.12f  ylo yhi\n%.12f\t%.12f  zlo zhi\n\n",
      ...
      int sp_count = 0;
      for(const auto& s : *species) { species_header += onika::format_string("%12llu %.12f # %s\n",++sp_count,s.m_mass,s.name()); }
      species_header += "\n\nAtoms # atomic\n\n";
	    ...
	    for(size_t c=0; c<n_cells;++c)
      {
        if( !grid->is_ghost_cell(c) )
        {
          ...
          for(int p=0;p<np;++p)
            ...
            int atom_id = 0;
            atom_id = ids[p];
            auto data_line = onika::format_string(data_line_format, atom_id+1 , atom_type+1 , r.x,r.y,r.z );
	    ...
	    for(size_t c=0; c<n_cells;++c)
      {
        if( !grid->is_ghost_cell(c) )
        {
	        ...
	        const auto * __restrict__ vz = cells[c][field::vz];
		      ...
		      for(int p=0;p<np;++p)
	        {
	          auto data_line = onika::format_string(vel_line_format, global_atom_start+atom_count+1 , vx[p] , vy[p] , vz[p] );
