...
/*           ,class = AssertGridHasFields< GridT , field::_rx0, field::_ry0, field::_rz0 > */
  ...
  class WriteXYZ : public OperatorNode
  {
    ...
    ADD_SLOT( std::string        , filename            , INPUT , std::string("output.xyz") , DocString{"File name"} );
...
/*
250  atoms
Lattice="15.900000000000 0.000000000000 0.000000000000 0.000000000000 15.900000000000 0.000000000000 0.000000000000 0.000000000000 15.900000000000"
...
*/
      ...
      auto [ hxx, hxy, hxz, hyx, hyy, hyz, hzx, hzy, hzz ] = transpose(domain->xform() * diag_matrix(domain->extent()-domain->origin()));
      
      std::string domain_header = format_string("%llu\nLattice=\"%.12f\t%.12f\t%.12f\t%.12f\t%.12f\t%.12f\t%.12f\t%.12f\t%.12f\"\n", static_cast<unsigned long long>(total_nb_particles),hxx,hxy,hxz,hyx,hyy,hyz,hzx,hzy,hzz);
      auto header = domain_header;
      unsigned long long offset_header = header.length();

      // only processor 0 writes the header
      if (rank==0)
      {
        ldbg << "XYZ: write header"<<std::endl;
      	MPI_File_write_at(mpiFile, 0 , header.data() , header.length() , MPI_CHAR , &status);
      }
      
	    // Write atom positions
      ldbg << "XYZ: write positions @"<<offset_header<< std::endl;
	    ...
	    for(size_t c=0; c<n_cells;++c)
      {
        if( !grid->is_ghost_cell(c) )
        {
	        ...
	        const size_t* __restrict__ ids = cells[c].field_pointer_or_null(field::id);
  ...
  template<class GridT> using WriteXYZTmpl = WriteXYZ<GridT>;
  
  // === register factories ===  
  CONSTRUCTOR_FUNCTION
  {
    OperatorNodeFactory::instance()->register_factory( "write_xyz", make_grid_variant_operator< WriteXYZTmpl > );
  }
