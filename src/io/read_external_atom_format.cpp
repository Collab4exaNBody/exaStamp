...
struct Box {
  ...
  void init(IOContext& ctx, Domain& domain, ReadBoundsSelectionMode bounds_mode) {
...
public:
  ...
  inline void execute () override final {
    ...
    lout << "======== Read external file format ========\n";
    ...
    Box box;
    IOContext io_ctx{};
    io_ctx.set<IOContext::REMAP_ATOM>(*remap_atom);
    
    if (this->species.has_value()) {
      for (size_t i = 0; i < species.size(); i++) {
        io_ctx.data.species.add(species.at(i).m_name, i + 1); // start indexing at 1
      }
    }
    ...
    if (rank == 0) {
      ...
      linfo("\nParsing done in %.0f ms | %.0f µs | %.0f ns\n", io_ctx.timer.x, io_ctx.timer.y, io_ctx.timer.z);
      box.init(io_ctx, domain, *bounds_mode);
    ...
    if (rank == 0) {
      ...
      for (size_t i = 0; i < io_ctx.data.particles.size(); ++i) {
        ParticleTupleIO& p = io_ctx.data.particles[i];
