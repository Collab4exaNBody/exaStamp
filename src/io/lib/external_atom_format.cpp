...
bool LAMMPSDataParser::parse_section_atoms(IOContext& ctx) {
  ...
  ctx.data.particles.resize(ctx.data.nat);
  auto parser = (ctx.has<IOContext::REMAP_ATOM>() ? atom_data_parsers<true>() : atom_data_parsers<false>())[style];
...
bool parse_keyword_atom_types(const TokenSet& tokens, IOContext& ctx) {
  ...
  if (!token_set_match_seq(tokens, NEEDLES_KEYWORD_ATOM_TYPES))
    return false;
  if (!parse_single_token(tokens.at<0>(), ctx.data.ntype))
    return false;
  return true;
}
...
inline bool parse_keyword_section(const TokenSet& tokens, IOContext&) {
  if (!tokens.atleast(1))
    return false;
  if (!token_match_any(tokens.at<0>(), NEEDLES_SECTIONS))
    return false;
  return true;
}
...
bool LAMMPSDumpParser::parse(IOContext& ctx) {
  ...
  auto parse_bbox = (properties.triclinic) ? parse_box_bounds_axis<true, matrix_traits::Mat3dSlice>
                                           : parse_box_bounds_axis<false, matrix_traits::Mat3dSlice>;

  Mat3d bbox;
  for (size_t i = 0; i < 3; ++i) {
    tokenize(trim_spaces(m_file.get_line()), m_tokens);
    if (!parse_bbox(m_tokens, matrix_traits::row(i, bbox))) {
      PANIC("Faild to parse ATOM: BOX BOUNDS");
    }
  }

  convert_bbox_to_lattice_vectors(ctx, bbox);

  // early return if we only care about the lattice vector (DEPMOD)
  if (ctx.has<IOContext::DOMAIN_ONLY>()) {
    return true;
  }
  ...
  if (!startswith(current_line(), label_atoms)) {
    PANIC("ITEM: ATOMS expected, found %s", std::string(current_line()));
  }

  tokenize(current_line(), m_tokens);
  parse_atom_properties(m_tokens, properties);

  // parse atom data
  size_t particle_count = 0;
  ctx.data.particles.resize(ctx.data.nat);

  auto parser = (!ctx.has<IOContext::REMAP_ATOM>() && properties.has<Field::ID>()) ? parse_atom_data<false>
...
bool ExtendedXYZParser::parse(IOContext& ctx) {
  ...
  if (!parse_extxyz_comment_line(current_line(), m_tokens, m_ptokens) ||
      !(parse_extxyz_lattice(m_tokens, ctx) && parse_xyz_properties(m_ptokens, m_properties))) {
  ...
  size_t particle_count = 0;
  ctx.data.particles.resize(ctx.data.nat);
  auto parser = (!ctx.has<IOContext::REMAP_ATOM>() && m_properties.has<Field::ID>()) ? parse_extxyz_atom_line<true>
                                                                                     : parse_extxyz_atom_line<false>;
  current_line() = trim_spaces(m_file.get_line());
  while (!m_file.eof() && particle_count < ctx.data.nat) {
...
bool read_external_atom_file(IOContext& ctx, std::string filepath, std::string format = "",
                             std::string compression = "") {

  // Ensure the file exists
  std::string abspath = std::string(std::filesystem::absolute(filepath));
  if (!std::filesystem::exists(abspath)) {
    PANIC("Input file doest not exists: '%s'\n", filepath.c_str());
  }

  auto infos = get_file_infos(filepath, format, compression);
  auto parser = infos.format.creator(filepath, FileMode::READ, infos.compression);
  if (parser == nullptr) {
    lerr << "could not create parser\n";
    return false;
  }
