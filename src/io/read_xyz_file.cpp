...
namespace exaStamp
{
  ...
  class ReadXYZNode : public OperatorNode
  {
...
//      double size_box = 0.0;
      ...
      if(rank==0)
      {
        ...
        box_size_x *= coord_conv;   // XYZ files use angstrom, exastamp uses nm
        box_size_y *= coord_conv;
        box_size_z *= coord_conv;
...
#     endif
  ...
  {
    OperatorNodeFactory::instance()->register_factory("read_xyz_file", make_grid_variant_operator< ReadXYZNode >);
  }
