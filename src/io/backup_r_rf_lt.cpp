...
namespace exaStamp
{
  ...
  template<class GridT, class = AssertGridHasFields< GridT, field::_id, field::_rxf, field::_ryf, field::_rzf> >
  
  class FilteredPositionBackupLongTerm : public OperatorNode
  {
    ADD_SLOT( MPI_Comm               , mpi         , INPUT , REQUIRED );
    ADD_SLOT( GridT  , grid     , INPUT );
    ADD_SLOT( Domain , domain            , INPUT , REQUIRED );  // 
    ADD_SLOT( PositionLongTermBackup , backup_r_rf_lt , INPUT_OUTPUT);
  ...
  public:
    ...
    {    
      ...
      backup_r_rf_lt->m_cell_offset[n_cells] = total_particles;
      backup_r_rf_lt->m_ids.assign( total_particles , uninitialized_id );
      backup_r_rf_lt->m_positions.assign( total_particles , Vec3d{0.,0.,0.} );
      backup_r_rf_lt->m_filtered_positions.assign( total_particles , Vec3d{0.,0.,0.} );      
      backup_r_rf_lt->m_xform = xform;
...
#     pragma omp parallel
      {
        GRID_OMP_FOR_BEGIN(dims-2*gl,_,loc, schedule(dynamic) reduction(min:idMin) reduction(max:idMax) reduction(+:inner_particles) )
        {
          ...
          const auto* __restrict__ rxf = cells[i][field::rxf];
          const auto* __restrict__ ryf = cells[i][field::ryf];
          const auto* __restrict__ rzf = cells[i][field::rzf];
          ...
          for(size_t j=0;j<n_particles;j++)
          {
            ...
            backup_r_rf_lt->m_ids[start+j] = pid;
            backup_r_rf_lt->m_positions[start+j] = Vec3d { rx[j] , ry[j] , rz[j] };
            backup_r_rf_lt->m_filtered_positions[start+j] = Vec3d { rxf[j] , ryf[j] , rzf[j] };	    
          }
      ...
      backup_r_rf_lt->m_idMin = idMin;
      backup_r_rf_lt->m_idMax = idMax+1; // exclusive max range is [idMin;idMax+1[

#     ifndef NDEBUG
      for(auto x : backup_r_rf_lt->m_ids)
      {
...
#     endif
  ...
  template<class GridT> using FilteredPositionBackupLongTermTmpl = FilteredPositionBackupLongTerm<GridT>;

 // === register factories ===  
  ONIKA_AUTORUN_INIT(backup_r_rf_lt)
  {
   OperatorNodeFactory::instance()->register_factory( "backup_r_rf_lt", make_grid_variant_operator< FilteredPositionBackupLongTermTmpl > );
  }
