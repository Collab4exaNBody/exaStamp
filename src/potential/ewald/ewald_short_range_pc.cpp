...
namespace exaStamp
{
  ...
  class EwaldShortRangePC : public OperatorNode
  {
    ...
    struct alignas(DEFAULT_ALIGNMENT) EwaldComputeBuffer
    {
    ...
    using ComputeBuffer = ComputePairBuffer2<false,false,EwaldComputeBuffer,CopyParticleCharge>;
  ...
  public:
    ...
    inline void execute () override final
    {
      ...
      //ldbg << "ncells="<< grid->number_of_cells() << ", rcut_max : "<< *rcut_max << " -> " << std::max( *rcut_max , rcut ) << std::endl;
    ...
    struct alignas(DEFAULT_ALIGNMENT) ForceOp 
    {
      ...
      inline void operator ()
        ...
        CellParticles* _unused
        ) const
      {
        FakeMat3d virial;
        this->operator () ( n,tab,ep,ax,ay,az,charge,virial,_unused);
      }
...
#       pragma omp simd reduction(+:_ep,_fx,_fy,_fz,_vir)
        for(size_t i=0;i<n;i++)
        {
          ...
          double e=0.0, de=0.0;
          ewald_compute_energy( m_params, charge, nbh_charge, r, e, de );
          de /= r;
