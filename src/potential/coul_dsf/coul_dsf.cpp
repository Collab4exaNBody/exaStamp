...
#include <exaStamp/potential/coul_dsf/coul_dsf.h>
...
namespace exaStamp
{
  ...
  template<
    ...
    >
  class CoulDsfPC : public OperatorNode
  {      
    // ========= I/O slots =======================
    ADD_SLOT( CoulDsfParms    , parameters        , INPUT , REQUIRED );
    ...
    struct alignas(DEFAULT_ALIGNMENT) CoulDsfComputeBuffer
    {
    ...
    using ComputeBuffer = ComputePairBuffer2<false,false,CoulDsfComputeBuffer,CopyParticleCharge>;
  ...
  public:
	...
	double cut_coul = m_params.rc;
	double cut_coulsq = cut_coul * cut_coul;    
	double erfcc = erfc(m_params.alpha * cut_coul);
	double erfcd = exp(-m_params.alpha * m_params.alpha * cut_coul * cut_coul);
...
#       pragma omp simd reduction(+:_ep,_fx,_fy,_fz,_vir)
        for(size_t i=0;i<n;i++)
        {
          ...
          double e=0.0, de=0.0;
	  coul_dsf_compute_energy( m_params, charge, nbh_charge, r, e, de );	  
          de /= r;
  ...
  template<class GridT> using CoulDsfPCTmpl = CoulDsfPC<GridT>;

  // === register factories ===  
  ONIKA_AUTORUN_INIT(coul_dsf)
  {  
    OperatorNodeFactory::instance()->register_factory( "coul_dsf_pc" , make_grid_variant_operator<CoulDsfPCTmpl> );
  }
