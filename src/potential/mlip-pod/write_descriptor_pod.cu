...
// Single combined, MPI-gathered per-atom export of compute_descriptor_pod's outputs -- direct
// structural copy of src/potential/snap/write_descriptor_snap.cu (same hand-rolled
// MPI_Gather+MPI_Gatherv pattern), renamed to POD's slot names. Regardless of how many MPI ranks
// own the simulation, this writes ONE file (from rank 0) with one row per globally-owned
// (non-ghost) atom, user-selectable columns.
namespace exaStamp
{
  using namespace exanb;

  template<class GridT>
  class WriteDescriptorPod : public OperatorNode
  {
    ADD_SLOT( MPI_Comm , mpi  , INPUT , REQUIRED );
    ADD_SLOT( GridT    , grid , INPUT , REQUIRED );
    ADD_SLOT( onika::memory::CudaMMVector<double> , pod_descriptors , INPUT , OPTIONAL , DocString{"see compute_descriptor_pod; required if 'descriptor' is in fields"} );
    ADD_SLOT( long     , ncoeff , INPUT , OPTIONAL , DocString{"see compute_descriptor_pod; required if 'descriptor' or 'derivative' is in fields"} );
    ADD_SLOT( std::string , deriv_agg_field_prefix , INPUT , std::string("pda_")
            , DocString{"Must match compute_descriptor_pod's own deriv_agg_field_prefix; required if 'derivative' is in fields. Run update_opt_from_ghost on these fields first on a multi-rank run -- see compute_descriptor_pod's documentation."} );
  ...
  public:
    inline void execute() override final
    {
      ...
      {
        fatal_error() << "write_descriptor_pod: unknown format '"<<*format<<"' (choices: text, npy)" << std::endl;
      }
      ...
      for( const auto & f : *fields )
      {
        ...
        else fatal_error() << "write_descriptor_pod: unknown field '"<<f<<"' (choices: id, x, y, z, descriptor, derivative)" << std::endl;
      }
      if( want_desc && ( ! pod_descriptors.has_value() || ! ncoeff.has_value() ) )
      {
        fatal_error() << "write_descriptor_pod: 'descriptor' requested but pod_descriptors/ncoeff unavailable -- run compute_descriptor_pod first" << std::endl;
      }
      if( want_deriv && ! ncoeff.has_value() )
      {
        fatal_error() << "write_descriptor_pod: 'derivative' requested but ncoeff unavailable -- run compute_descriptor_pod with compute_derivative: true first" << std::endl;
      }
      ...
      if( want_deriv )
      {
        ...
        for( long k=0; k<nc*3; k++ )
        {
          ...
          {
            fatal_error() << "write_descriptor_pod: field '"<<*deriv_agg_field_prefix<<k<<"' not found -- run compute_descriptor_pod with compute_derivative: true first" << std::endl;
          }
        }
      }
      ...
      if( *format == "npy" )
      {
        ...
        // float64 -- exact for any realistic atom count) -- matches write_descriptor_pod_global's
        // own single-array .npy shape, rather than one file per field.
        long total_width = 0;
        for( const auto & f : *fields )
        {
...
user-selectable columns, in the order given by `fields`:
  ...
  - id           : field::id
  - x, y, z      : position
  - descriptor   : compute_descriptor_pod's POD descriptor vector, ncoeff values
  - derivative   : compute_descriptor_pod's compact per-atom descriptor-derivative aggregate,
                   ...
                   on them first on a multi-rank run (see compute_descriptor_pod's doc).

Note: unlike SNAP, there is no compact LAMMPS reference to cross-validate this aggregate against
at full system scale -- LAMMPS's own `compute podd/atom` is a dense per-atom-id matrix that scales
with total atom count (see compute_descriptor_pod's documentation), so cross-validation of this
aggregate against real LAMMPS is only practical on small test systems (see
data/regression_new/analysis_particle/test_pod_descriptors/compare_derivative.py).
...
update_opt_from_ghost: { opt_fields: [ "pda_.*" ] }   # only needed on more than one rank
write_descriptor_pod:
  ...
  ONIKA_AUTORUN_INIT(write_descriptor_pod)
  {
    OperatorNodeFactory::instance()->register_factory( "write_descriptor_pod", make_grid_variant_operator< WriteDescriptorPod > );
  }
