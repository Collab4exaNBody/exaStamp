...
// ── File-parsing helpers (replace LAMMPS Tokenizer / utils) ──────────────────

static std::string pod_trim_comment(const std::string &line) {
    auto pos = line.find('#');
    return (pos != std::string::npos) ? line.substr(0, pos) : line;
}

static std::vector<std::string> pod_split_words(const std::string &line) {
    std::istringstream iss(pod_trim_comment(line));
...
static int pod_count_words(const std::string &line) {
    return (int)pod_split_words(line).size();
}

// constructor
EAPOD::EAPOD(const std::string &pod_file, const std::string &coeff_file) :
    elemindex(nullptr), Phi(nullptr), Lambda(nullptr), coeff(nullptr),
...
EAPOD::~EAPOD()
{
  delete[] elemindex; elemindex = nullptr;
  delete[] Phi; Phi = nullptr;
  delete[] Lambda; Lambda = nullptr;
  delete[] Proj; Proj = nullptr;
  delete[] Centroids; Centroids = nullptr;
  delete[] bd; bd = nullptr;
  delete[] bdd; bdd = nullptr;
  delete[] pd; pd = nullptr;
  delete[] pdd; pdd = nullptr;
  delete[] coeff; coeff = nullptr;
  delete[] tmpmem; tmpmem = nullptr;
  delete[] tmpint; tmpint = nullptr;
  delete[] pn3; pn3 = nullptr;
  delete[] pq3; pq3 = nullptr;
  delete[] pc3; pc3 = nullptr;
  delete[] pa4; pa4 = nullptr;
  delete[] pb4; pb4 = nullptr;
  delete[] pc4; pc4 = nullptr;
  delete[] pq4; pq4 = nullptr;
  delete[] ind23; ind23 = nullptr;
  delete[] ind32; ind32 = nullptr;
  delete[] ind33; ind33 = nullptr;
  delete[] ind34; ind34 = nullptr;
  delete[] ind43; ind43 = nullptr;
  delete[] ind44; ind44 = nullptr;
  delete[] ind33l; ind33l = nullptr;
  delete[] ind34l; ind34l = nullptr;
  delete[] ind44l; ind44l = nullptr;
  delete[] ind33r; ind33r = nullptr;
  delete[] ind34r; ind34r = nullptr;
  delete[] ind44r; ind44r = nullptr;
  delete[] soa_rij; soa_rij = nullptr;
  delete[] soa_fij; soa_fij = nullptr;
  delete[] soa_ti;  soa_ti  = nullptr;
  delete[] soa_tj;  soa_tj  = nullptr;
}

void EAPOD::read_pod_file(const std::string &pod_file)
{
  std::ifstream fp(pod_file);
  if (!fp.is_open())
    throw std::runtime_error("Cannot open POD file: " + pod_file);

  std::string line;
  while (std::getline(fp, line)) {
    std::vector<std::string> words = pod_split_words(line);
    if (words.empty()) continue;

    const auto &keywd = words[0];

    if (keywd == "species") {
      nelements = (int)words.size() - 1;
      for (int ielem = 1; ielem <= nelements; ielem++)
        species.push_back(words[ielem]);
    } else if (keywd == "pbc") {
      if (words.size() != 4)
        throw std::runtime_error("Improper POD file: pbc expects 3 values");
      pbc[0] = std::stoi(words[1]);
      pbc[1] = std::stoi(words[2]);
      pbc[2] = std::stoi(words[3]);
    } else if (keywd != "#") {
      if (words.size() != 2)
      ...
      if (keywd == "inverse_polynomial_degree")             inversedegree= std::stoi(words[1]);
      if (keywd == "onebody")                               onebody      = std::stoi(words[1]);
      if (keywd == "twobody_number_radial_basis_functions") nrbf2        = std::stoi(words[1]);
      if (keywd == "threebody_number_radial_basis_functions")nrbf3       = std::stoi(words[1]);
      if (keywd == "threebody_angular_degree")              P3           = std::stoi(words[1]);
      if (keywd == "fourbody_number_radial_basis_functions")nrbf4        = std::stoi(words[1]);
      if (keywd == "fourbody_angular_degree")               P4           = std::stoi(words[1]);
      if (keywd == "fivebody_number_radial_basis_functions")nrbf33       = std::stoi(words[1]);
      if (keywd == "fivebody_angular_degree")               P33          = std::stoi(words[1]);
      if (keywd == "sixbody_number_radial_basis_functions") nrbf34       = std::stoi(words[1]);
      if (keywd == "sixbody_angular_degree")                P34          = std::stoi(words[1]);
      if (keywd == "sevenbody_number_radial_basis_functions")nrbf44      = std::stoi(words[1]);
  ...
  if (nrbf4 < nrbf34) throw std::runtime_error("POD: nrbf34 must be <= nrbf4");
  if (nrbf4 < nrbf44) throw std::runtime_error("POD: nrbf44 must be <= nrbf4");
  nrbfmax = nrbf2;
  nrbfmax = (nrbfmax < nrbf3)  ? nrbf3  : nrbfmax;
  ...
  if (P3 < P4)  throw std::runtime_error("POD: P4 must be <= P3");
  if (P4 < P33) throw std::runtime_error("POD: P33 must be <= P4");
  if (P4 < P34) throw std::runtime_error("POD: P34 must be <= P4");
  if (P4 < P44) throw std::runtime_error("POD: P44 must be <= P4");
  if (P3 > 12)  throw std::runtime_error("POD: P3 must be <= 12");
  if (P4 > 6)   throw std::runtime_error("POD: P4 must be <= 6");

  int Ne = nelements;
  elemindex = new int[Ne*Ne]();
  int k = 0;
  ...
  if (nrbf33 > 0) {
    ...
    ind33l = new int[nld33](); ind33r = new int[nld33]();
  ...
  if (nrbf34 > 0) {
    ...
    ind34l = new int[nld34](); ind34r = new int[nld34]();
  ...
  if (nrbf44 > 0) {
    ...
    ind44l = new int[nld44](); ind44r = new int[nld44]();
    crossindices(ind44l,ind44r,dabf4,nabf4,nrbf4,nebf4,dabf4,nabf4,nrbf4,nebf4,P44,nrbf44);
  }
  ngd33 = nld33*Ne; ngd34 = nld34*Ne; ngd44 = nld44*Ne;
  nl33 = nld33;     nl34 = nld34;     nl44 = nld44;
  nd33 = ngd33;     nd34 = ngd34;     nd44 = ngd44;
...
{
  std::ifstream fp(coeff_file);
  if (!fp.is_open())
    throw std::runtime_error("Cannot open POD coefficient file: " + coeff_file);

  // Read first non-blank, non-comment line: "label ncoeff nproj ncent"
  std::string line;
  std::vector<std::string> header;
  while (std::getline(fp, line)) {
    header = pod_split_words(line);
    if (!header.empty()) break;
  }
  if (header.size() != 4)
    throw std::runtime_error("Incorrect format in POD coefficient file (expected: label ncoeff nproj ncent)");

  const int ncoeffall = std::stoi(header[1]);
  const int nprojall  = std::stoi(header[2]);
  const int ncentall  = std::stoi(header[3]);

  auto read_doubles = [&](double *arr, int n, const char *what) {
    for (int i = 0; i < n; i++) {
      while (std::getline(fp, line)) {
        auto w = pod_split_words(line);
        if (w.empty()) continue;
        if ((int)w.size() != 1)
          throw std::runtime_error(std::string("Incorrect format in POD coefficient file (") + what + ")");
        arr[i] = std::stod(w[0]);
        break;
      }
    }
  ...
  coeff     = new double[ncoeffall]();
  Proj      = new double[nprojall]();
  Centroids = new double[ncentall]();

  read_doubles(coeff,     ncoeffall, "coeff");
  read_doubles(Proj,      nprojall,  "proj");
  read_doubles(Centroids, ncentall,  "centroids");

  if (ncoeffall != nCoeffAll)
    throw std::runtime_error("POD: coefficient count mismatch (file has " +
                             std::to_string(ncoeffall) + ", expected " + std::to_string(nCoeffAll) + ")");
  if (nClusters > 1) {
    if (nprojall != nComponents*Mdesc*nelements)
      throw std::runtime_error("POD: projection matrix size mismatch");
    if (ncentall != nComponents*nClusters*nelements)
      throw std::runtime_error("POD: centroids size mismatch");
  }
}
...
int EAPOD::read_coeff_file(const std::string &coeff_file)
{
  std::ifstream fp(coeff_file);
  if (!fp.is_open())
    throw std::runtime_error("Cannot open POD coeff file: " + coeff_file);
  ...
  if (header.size() != 2)
    throw std::runtime_error("Incorrect format in POD coeff file (expected: label ncoeff)");
  ...
  for (int i = 0; i < ncoeffall; i++) {
    while (std::getline(fp, line)) {
      ...
      if (w.empty()) continue;
      if ((int)w.size() != 1)
        throw std::runtime_error("Incorrect format in POD coeff file");
      coeff[i] = std::stod(w[0]);
      break;
    }
  }
  return ncoeffall;
...
{
  std::ifstream fp(proj_file);
  if (!fp.is_open())
    throw std::runtime_error("Cannot open PCA projection file: " + proj_file);
  ...
  if (header.size() != 2)
    throw std::runtime_error("Incorrect format in PCA projection file (expected: label nproj)");

  const int nprojall = std::stoi(header[1]);
  Proj = new double[nprojall]();

  for (int i = 0; i < nprojall; i++) {
    while (std::getline(fp, line)) {
      auto w = pod_split_words(line);
      if (w.empty()) continue;
      if ((int)w.size() != 1)
        throw std::runtime_error("Incorrect format in PCA projection file");
      Proj[i] = std::stod(w[0]);
      break;
    }
  }
...
{
  std::ifstream fp(centroids_file);
  if (!fp.is_open())
    throw std::runtime_error("Cannot open PCA centroids file: " + centroids_file);
  ...
  if (header.size() != 2)
    throw std::runtime_error("Incorrect format in PCA centroids file (expected: label ncent)");

  const int ncentall = std::stoi(header[1]);
  Centroids = new double[ncentall]();

  for (int i = 0; i < ncentall; i++) {
    while (std::getline(fp, line)) {
      auto w = pod_split_words(line);
      if (w.empty()) continue;
      if ((int)w.size() != 1)
        throw std::runtime_error("Incorrect format in PCA centroids file");
      Centroids[i] = std::stod(w[0]);
      break;
    }
  }
  return ncentall;
...
void EAPOD::init2body()
{
  ...
  Phi = new double[ns * ns]();
  Lambda = new double[ns]();
...
void EAPOD::init3body(int Pa3)
{
  ...
  pn3 = new int[nabf3+1](); // array stores the number of monomials for each degree
  pq3 = new int[K3*2](); // array needed for the recursive computation of the angular basis functions
  pc3 = new int[K3]();   // array needed for the computation of the three-body descriptors
...
void EAPOD::init4body(int Pa4)
{
  ...
  int *pn4, *tm4;
  pn4 = new int[Pa4+2](); // array stores the number of monomials for each degree
  pq4 = new int[K4*2]();  // array needed for the recursive computation of the angular basis functions
  tm4 = new int[K4]();
  ...
  // Allocate memory for the coefficients, the basis functions, and the cutoff function
  pa4 = new int[nabf4+1](); // this array is a subset of the array ns
  pb4 = new int[Q4*3](); // array stores the indices of the monomials needed for the computation of the angular basis functions
  pc4 = new int[Q4]();   // array of monomial coefficients needed for the computation of the four-body descriptors

  // Initialize the arrays
  init4bodyarray(pa4, pb4, pc4, Pa4);

  // Deallocate memory
  delete[] pn4; pn4 = nullptr;
  delete[] tm4; tm4 = nullptr;
}
...
void EAPOD::allocate_temp_memory(int Nj)
{
  ...
  soa_rij = new double[3*Nj]();
  soa_fij = new double[3*Nj]();
  soa_ti  = new int[Nj]();
  soa_tj  = new int[Nj]();
}
