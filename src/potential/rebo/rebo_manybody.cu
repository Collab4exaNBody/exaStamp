...
namespace exaStamp
{
  ...
  // Many-body pass: attractive term VA scaled by the bond order bij
  // (REBOManyBodyForceOp, see rebo_force_op_cached.h) — three-body sigma-pi
  // sums, piRC, and the four-body Tij dihedral double loop. Reads the
  // precomputed NijC/NijH/Nconj coordination fields. Split out from the pure
  // pairwise pass (rebo_pairwise.cu) so the two can be run and profiled as
  // separate kernels.
  template <class GridT, class = AssertGridHasFields<GridT, field::_ep, field::_fx, field::_fy, field::_fz>> class ReboManyBodyForce : public OperatorNode
  {
    ...
    inline void execute() override final
    {
      ...
      auto with_locks = [&](auto cp_locks) {
        compute_cell_particle_pairs(*grid, *rebo_cutoff, *ghost, make_compute_pair_optional_args(nbh_it, cp_weight, cp_xform, cp_locks), compute_buf_rebo,
                                     REBOManyBodyForceOp<decltype(nijc_acc), decltype(nijh_acc), decltype(nconj_acc)>{params_ro, nijc_acc, nijh_acc, nconj_acc}, manybody_op_fields, parallel_execution_context());
      };
      if (use_locks)
  ...
  template <class GridT> using ReboManyBodyForceTmpl = ReboManyBodyForce<GridT>;

  ONIKA_AUTORUN_INIT(rebo_manybody) { OperatorNodeFactory::instance()->register_factory("rebo_manybody", make_grid_variant_operator<ReboManyBodyForceTmpl>); }
