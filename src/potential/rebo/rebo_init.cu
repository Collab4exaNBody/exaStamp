...
namespace exaStamp
{
  ...
  class ReboInit : public OperatorNode
  {
    ADD_SLOT( ReboParams       , parameters , INPUT_OUTPUT , REQUIRED );
  ...
  public:
    ...
    {
      ldbg << "Initializing Rebo potential" << std::endl;
      ...
      *rcut_max = std::max({ *rcut_max,
      ...
      lout << "REBO bondorder_cutoff = " << *bondorder_cutoff << std::endl;
      lout << "REBO rebo_cutoff      = " << *rebo_cutoff      << std::endl;
      lout << "REBO rcut_max         = " << *rcut_max         << std::endl;
    }

  };

  ONIKA_AUTORUN_INIT(rebo_init)
  {
    OperatorNodeFactory::instance()->register_factory("rebo_init", make_simple_operator<ReboInit>);
  }
