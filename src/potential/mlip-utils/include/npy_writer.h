...
namespace exaStamp
{
  ...
  // little-endian host (true for every platform this code targets). Shared header-only across every
  // ML potential family's write_descriptor_<family>_global.cu (POD/SNAP/k2b/MTP), header-only so no
  // link dependency / CMake registration needed -- see this directory's own lack of a CMakeLists.txt.
