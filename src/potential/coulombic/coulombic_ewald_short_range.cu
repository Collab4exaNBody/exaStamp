...
namespace exaStamp
{
  ...
  struct EwaldShortRangeComputeContext;

  template<> struct EwaldShortRangeComputeContext<false,false>
  {
  ...
  template<> struct EwaldShortRangeComputeContext<true,false>
  {
  ...
  template<> struct EwaldShortRangeComputeContext<true,true>
  {
    static inline constexpr bool ComputeEnergy = true;
  ...
  struct EwaldShortRangeForceOp
  {
    ...
    const EwaldParameters m_params;
      ...
      CellParticlesT cells,size_t cell_b,size_t p_b, double weight ) const
    {
      ...
      double e=0.0, de=0.0;
      ewald_compute_energy( m_params, ctx.ext.charge_a * charge_b, r, e, de );
...
namespace exanb
{
  ...
  struct ComputePairTraits< exaStamp::EwaldShortRangeForceOp<CPLocksT,ChargeFieldT,TypeFieldT,VirialFieldT,_PerAtomCharge,_UseSymetry,_ComputeEnergy,_ComputeVirial> >
  {
...
namespace exaStamp
{
  ...
  public:
    ...
    inline void execute () override final
    {
      ...
      const double rcut = ewald_config->radius;
...
//        [[maybe_unused]] static constexpr onika::parallel::AssertFunctorSizeFitIn< alignof(force_op) , 1 , decltype(force_op) > _check_functor_size = {};
      ...
      auto compute_force_energy_opt_weights = [&](auto & cp_locks, auto && cp_weight)
      {
        ...
        if( log_energy )
        {
          using ForceOp = EwaldShortRangeForceOp<decltype(cp_locks),ChargeFieldT,TypeFieldT,VirialFieldT,true,false,true,true>;
          using CPBufT = ComputePairBuffer2<false,false, EwaldShortRangeComputeContext<true,true> >;
          compute_force_energy( cp_locks, cp_weight, ForceOp{*ewald_config, species->data(), cp_locks, charge_field, type_field, virial_field} , make_compute_pair_buffer<CPBufT>() );
        }
        else
        {
          using ForceOp = EwaldShortRangeForceOp<decltype(cp_locks),ChargeFieldT,TypeFieldT,VirialFieldT,true,false,false,false>;
          using CPBufT = ComputePairBuffer2<false,false, EwaldShortRangeComputeContext<false,false> >;
          compute_force_energy( cp_locks, cp_weight, ForceOp{*ewald_config, species->data(), cp_locks, charge_field, type_field, virial_field} , make_compute_pair_buffer<CPBufT>() );
        }
      };
