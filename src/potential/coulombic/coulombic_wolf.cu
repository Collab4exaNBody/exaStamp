...
namespace exaStamp
{
  ...
  struct WolfComputeContext;

  template<> struct WolfComputeContext<false,false>
  {
  ...
  template<> struct WolfComputeContext<true,false>
  {
  ...
  template<> struct WolfComputeContext<true,true>
  {
    static inline constexpr bool ComputeEnergy = true;
  ...
  struct WolfForceOp
  {
      ...
      CellParticlesT cells,size_t cell_b,size_t p_b, double weight ) const
    {
      ...
      double e=0.0, de=0.0;
      wolf_compute_energy( m_params, ctx.ext.charge_a * charge_b, r, e, de );
...
namespace exanb
{
  ...
  struct ComputePairTraits< exaStamp::WolfForceOp<CPLocksT,ChargeFieldT,TypeFieldT,VirialFieldT,_PerAtomCharge,_UseSymetry,_ComputeEnergy,_ComputeVirial> >
  {
...
namespace exaStamp
{
  ...
  template<
    ...
    >
  class WolfPC : public OperatorNode
  {
    // ========= I/O slots =======================
...
//        [[maybe_unused]] static constexpr onika::parallel::AssertFunctorSizeFitIn< alignof(force_op) , 1 , decltype(force_op) > _check_functor_size = {};
      ...
      auto compute_force_energy_opt_weights = [&](auto & cp_locks, auto && cp_weight)
      {
        ...
        if( log_energy )
        {
          using ForceOp = WolfForceOp<decltype(cp_locks),ChargeFieldT,TypeFieldT,VirialFieldT,true,false,true,true>;
          using CPBufT = ComputePairBuffer2<false,false, WolfComputeContext<true,true> >;
        ...
        {
          using ForceOp = WolfForceOp<decltype(cp_locks),ChargeFieldT,TypeFieldT,VirialFieldT,true,false,false,false>;
          using CPBufT = ComputePairBuffer2<false,false, WolfComputeContext<false,false> >;
          compute_force_energy( cp_locks, cp_weight, ForceOp{*parameters, species->data(), cp_locks, charge_field, type_field, virial_field} , make_compute_pair_buffer<CPBufT>() );
        }
      };
  ...
  template<class GridT> using WolfPCTmpl = WolfPC<GridT>;

  // === register factories ===  
  ONIKA_AUTORUN_INIT(coulombic_wolf)
  {  
    OperatorNodeFactory::instance()->register_factory( "coulombic_wolf" , make_grid_variant_operator<WolfPCTmpl> );
  }
