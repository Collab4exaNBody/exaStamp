...
//      , Gc( p.Gc.data() )
  ...
  ONIKA_HOST_DEVICE_FUNC static inline void ewald_compute_energy(const EwaldParametersT& p, double c, double r, double& e, double& de)
  {
    ...
    const double cf = p.gm_sr * c; // (1/(4.pi.epsilon0)qi.qj

    e = cf * erfc(ar) / r;
    de = - (cf * p.bt_sr * exp(-ar * ar) + e) / r;
  }

  inline void ewald_init_parameters(double alpha, double radius, double epsilon, unsigned int in_kmax, const Vec3d& domainSize, EwaldParameters& p , ::exanb::LogStreamWrapper& ldbg )
  {
...
//    p.Gc.resize( p.nk );
  ...
  inline void ewald_init_parameters(double alpha, double radius, double epsilon, unsigned int in_kmax, const Vec3d& domainSize, EwaldParameters& p )
  {
...
// specialize ReadOnlyShallowCopyType so ReadOnlyEwaldParameters is the read only type for EwaldParameters
namespace onika { namespace cuda {
  template<> struct ReadOnlyShallowCopyType< exaStamp::EwaldParameters > { using type = exaStamp::ReadOnlyEwaldParameters; };
} }
