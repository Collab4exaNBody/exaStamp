...
#include <exaStamp/potential/coulombic/dsf.h>
...
namespace exaStamp
{
  ...
  struct DsfComputeContext;

  template<> struct DsfComputeContext<false,false>
  {
  ...
  template<> struct DsfComputeContext<true,false>
  {
  ...
  template<> struct DsfComputeContext<true,true>
  {
    static inline constexpr bool ComputeEnergy = true;
  ...
  // Dsf Compute functor
  struct DsfSelfForceOp
  {
    const ParticleSpecie * m_species = nullptr;
    const DsfParameters m_params = {};
  ...
  struct DsfForceOp
  {
      ...
      CellParticlesT cells,size_t cell_b,size_t p_b, double weight ) const
    {
      ...
      double e=0.0, de=0.0;
      dsf_compute_energy( m_params, ctx.ext.charge_a * charge_b, r, e, de );
...
namespace exanb
{
  ...
  struct ComputePairTraits< exaStamp::DsfForceOp<CPLocksT,ChargeFieldT,TypeFieldT,VirialFieldT,_PerAtomCharge,_UseSymetry,_ComputeEnergy,_ComputeVirial> >
  {
...
namespace exaStamp
{
  ...
  template<
    ...
    >
  class DsfPC : public OperatorNode
  {
    // ========= I/O slots =======================
    ADD_SLOT( DsfParameters        , parameters          , INPUT , REQUIRED );
  ...
  public:
    ...
    inline void execute () override final
    {
      ...
      *rcut_max = std::max( *rcut_max , rcut );
      ...
      if( log_energy )
        {
          DsfSelfForceOp func = { species->data() , *parameters , species->at(0).m_charge };
...
//        [[maybe_unused]] static constexpr onika::parallel::AssertFunctorSizeFitIn< alignof(force_op) , 1 , decltype(force_op) > _check_functor_size = {};
      ...
      auto compute_force_energy_opt_weights = [&](auto & cp_locks, auto && cp_weight)
      {
        ...
        if( log_energy )
        {
          using ForceOp = DsfForceOp<decltype(cp_locks),ChargeFieldT,TypeFieldT,VirialFieldT,true,false,true,true>;
          using CPBufT = ComputePairBuffer2<false,false, DsfComputeContext<true,true> >;
        ...
        {
          using ForceOp = DsfForceOp<decltype(cp_locks),ChargeFieldT,TypeFieldT,VirialFieldT,true,false,false,false>;
          using CPBufT = ComputePairBuffer2<false,false, DsfComputeContext<false,false> >;
          compute_force_energy( cp_locks, cp_weight, ForceOp{*parameters, species->data(), cp_locks, charge_field, type_field, virial_field} , make_compute_pair_buffer<CPBufT>() );
        }
      };
  ...
  template<class GridT> using DsfPCTmpl = DsfPC<GridT>;

  // === register factories ===  
  ONIKA_AUTORUN_INIT(coulombic_dsf)
  {  
    OperatorNodeFactory::instance()->register_factory( "coulombic_dsf" , make_grid_variant_operator<DsfPCTmpl> );
  }
