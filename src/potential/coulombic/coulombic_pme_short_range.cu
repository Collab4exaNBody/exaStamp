...
/*
================================================================================
PME Short-Range (Real-Space) Electrostatics Operator
================================================================================

This operator computes the short-range (real-space) part of PME electrostatics:
  E_short = Σᵢ<ⱼ qᵢqⱼ erfc(α*rᵢⱼ) / rᵢⱼ

This is identical to the Ewald short-range term and can be computed with
existing neighbor list infrastructure.

The splitting parameter α is the same as used in the PME long-range operator.
...
namespace exaStamp
{
  ...
  struct PMEShortRangeComputeContext;

  template<> struct PMEShortRangeComputeContext<false,false>
  {
  ...
  template<> struct PMEShortRangeComputeContext<true,false>
  {
  ...
  template<> struct PMEShortRangeComputeContext<true,true>
  {
    static inline constexpr bool ComputeEnergy = true;
  ...
  struct PMEShortRangeForceOp
  {
      ...
      CellParticlesT cells,size_t cell_b,size_t p_b, double weight ) const
    {
      ...
      double e=0.0, de=0.0;
      pme_compute_short_range(p, ctx.ext.charge_a * charge_b, r, e, de);
      de /= r;
  ...
  //============================================================================
  // Self-energy correction functor (removes spurious self-interaction)
  //============================================================================
  
  struct PMESelfEnergyFunc
  {
    const ParticleSpecie* __restrict__ m_species = nullptr;
    ReadOnlyPMEParameters p;
    ...
    inline void operator()(double& ep, unsigned int type) const
    {
      ...
      // E_self = -α/√π * q²
      ep -= 1. / fpe0 * p.alpha / std::sqrt(M_PI) * q * q;
    }
  };
...
namespace exanb
{
  ...
  struct ComputePairTraits< exaStamp::PMEShortRangeForceOp<CPLocksT,ChargeFieldT,TypeFieldT,VirialFieldT,_PerAtomCharge,_UseSymetry,_ComputeEnergy,_ComputeVirial> >
  {
  ...
  template<>
  struct ComputeCellParticlesTraits<exaStamp::PMESelfEnergyFunc>
  {
    static inline constexpr bool RequiresBlockSynchronousCall = false;
    static inline constexpr bool CudaCompatible = true;
  };
...
namespace exaStamp
{
  ...
  >
  class PMEShortRangePC : public OperatorNode
  {
    // I/O Slots
    ...
    using pme_self_force_field_set_t = FieldSet<field::_ep,field::_type>;    
    static constexpr pme_self_force_field_set_t pme_self_force_field_set = {};
  ...
  public:
    ...
    inline void execute() override final
    {
      ...
      const double rcut = pme_config->rcut;
...
//        [[maybe_unused]] static constexpr onika::parallel::AssertFunctorSizeFitIn< alignof(force_op) , 1 , decltype(force_op) > _check_functor_size = {};
      ...
      auto compute_force_energy_opt_weights = [&](auto & cp_locks, auto && cp_weight)
      {
        ...
        if( log_energy )
        {
          using ForceOp = PMEShortRangeForceOp<decltype(cp_locks),ChargeFieldT,TypeFieldT,VirialFieldT,true,false,true,true>;
          using CPBufT = ComputePairBuffer2<false,false, PMEShortRangeComputeContext<true,true> >;
          compute_force_energy( cp_locks, cp_weight, ForceOp{*pme_config, species->data(), cp_locks, charge_field, type_field, virial_field} , make_compute_pair_buffer<CPBufT>() );
        }
        else
        {
          using ForceOp = PMEShortRangeForceOp<decltype(cp_locks),ChargeFieldT,TypeFieldT,VirialFieldT,true,false,false,false>;
          using CPBufT = ComputePairBuffer2<false,false, PMEShortRangeComputeContext<false,false> >;
          compute_force_energy( cp_locks, cp_weight, ForceOp{*pme_config, species->data(), cp_locks, charge_field, type_field, virial_field} , make_compute_pair_buffer<CPBufT>() );
        }
      };
      ...
      if ( log_energy ) {
        PMESelfEnergyFunc self_func = { species->data() , *pme_config };
        compute_cell_particles( *grid , false , self_func , pme_self_force_field_set , parallel_execution_context() );
      }
      
    //   ldbg << "------------------------" << std::endl;
    //   ldbg << "End PME Short-Range" << std::endl;
    //   ldbg << "------------------------" << std::endl;
    }
  };

  template<class GridT> using PMEShortRangePCTmpl = PMEShortRangePC<GridT>;

  // Register factory
  ONIKA_AUTORUN_INIT(coulombic_pme_short_range)
  {
    OperatorNodeFactory::instance()->register_factory(
      "coulombic_pme_short_range",
      make_grid_variant_operator<PMEShortRangePCTmpl>
    );
  }
