...
#include <exaStamp/potential/coulombic/rf.h>
...
namespace exaStamp
{
  ...
  struct ReactionFieldForceOp
  {
    ...
    const ReactionFieldParameters m_params;
      ...
      CellParticlesT cells,size_t cell_b,size_t p_b, double weight ) const
    {
      ...
      double e=0.0, de=0.0;
      rf_compute_energy( m_params, ctx.ext.charge_a * charge_b, r, e, de );
...
namespace exanb
{
  ...
  struct ComputePairTraits< exaStamp::ReactionFieldForceOp<CPLocksT,ChargeFieldT,TypeFieldT,VirialFieldT,_PerAtomCharge,_UseSymetry,_ComputeEnergy,_ComputeVirial> >
  {
...
namespace exaStamp
{
  ...
  {
    // ========= I/O slots =======================
    ADD_SLOT( ReactionFieldParameters        , parameters          , INPUT , REQUIRED );
...
//        [[maybe_unused]] static constexpr onika::parallel::AssertFunctorSizeFitIn< alignof(force_op) , 1 , decltype(force_op) > _check_functor_size = {};
      ...
      auto compute_force_energy_opt_weights = [&](auto & cp_locks, auto && cp_weight)
      {
        ...
        if( log_energy )
        {
          using ForceOp = ReactionFieldForceOp<decltype(cp_locks),ChargeFieldT,TypeFieldT,VirialFieldT,true,false,true,true>;
        ...
        {
          using ForceOp = ReactionFieldForceOp<decltype(cp_locks),ChargeFieldT,TypeFieldT,VirialFieldT,true,false,false,false>;
  ...
  // === register factories ===  
  ONIKA_AUTORUN_INIT(coulombic_rf)
  {  
    OperatorNodeFactory::instance()->register_factory( "coulombic_rf" , make_grid_variant_operator<ReactionFieldPCTmpl> );
  }
