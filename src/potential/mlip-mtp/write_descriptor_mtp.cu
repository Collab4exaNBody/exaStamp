...
// Single combined, MPI-gathered per-atom export of compute_descriptor_mtp's outputs -- direct
// structural copy of mlip-pod/write_descriptor_pod.cu (same hand-rolled MPI_Gather+MPI_Gatherv
// pattern), renamed to MTP's slot names. Regardless of how many MPI ranks own the simulation,
// this writes ONE file (from rank 0) with one row per globally-owned (non-ghost) atom,
...
namespace exaStamp
{
  ...
  class WriteDescriptorMtp : public OperatorNode
  {
    ADD_SLOT( MPI_Comm , mpi  , INPUT , REQUIRED );
    ADD_SLOT( GridT    , grid , INPUT , REQUIRED );
    ADD_SLOT( onika::memory::CudaMMVector<double> , mtp_descriptors , INPUT , OPTIONAL , DocString{"see compute_descriptor_mtp; required if 'descriptor' is in fields"} );
    ADD_SLOT( long     , ncoeff , INPUT , OPTIONAL , DocString{"see compute_descriptor_mtp; required if 'descriptor' or 'derivative' is in fields"} );
    ADD_SLOT( std::string , deriv_agg_field_prefix , INPUT , std::string("mda_")
            , DocString{"Must match compute_descriptor_mtp's own deriv_agg_field_prefix; required if 'derivative' is in fields. Run update_opt_from_ghost on these fields first on a multi-rank run -- see compute_descriptor_mtp's documentation."} );
    ...
    ADD_SLOT( std::string , filename , INPUT , std::string("mtp_descriptors.txt") , DocString{"Single combined output file, written once from rank 0 after gathering every rank's owned atoms. In 'npy' format a trailing '.txt' is stripped and '.npy' appended."} );
    ADD_SLOT( std::string , format , INPUT , std::string("text") , DocString{"Output format: 'text' (default, single combined plain-text file) or 'npy' (single combined 2-D float64 .npy v1.0 array, directly loadable via numpy.load())."} );
  ...
  public:
    inline void execute() override final
    {
      ...
      {
        fatal_error() << "write_descriptor_mtp: unknown format '"<<*format<<"' (choices: text, npy)" << std::endl;
      }
      ...
      for( const auto & f : *fields )
      {
        ...
        else fatal_error() << "write_descriptor_mtp: unknown field '"<<f<<"' (choices: id, x, y, z, descriptor, derivative)" << std::endl;
      }
      if( want_desc && ( ! mtp_descriptors.has_value() || ! ncoeff.has_value() ) )
      {
        fatal_error() << "write_descriptor_mtp: 'descriptor' requested but mtp_descriptors/ncoeff unavailable -- run compute_descriptor_mtp first" << std::endl;
      }
      if( want_deriv && ! ncoeff.has_value() )
      {
        fatal_error() << "write_descriptor_mtp: 'derivative' requested but ncoeff unavailable -- run compute_descriptor_mtp with compute_derivative: true first" << std::endl;
      }
      ...
      if( want_deriv )
      {
        ...
        for( long k=0; k<nc*3; k++ )
        {
          ...
          {
            fatal_error() << "write_descriptor_mtp: field '"<<*deriv_agg_field_prefix<<k<<"' not found -- run compute_descriptor_mtp with compute_derivative: true first" << std::endl;
          }
        }
      }
...
user-selectable columns, in the order given by `fields`:
  ...
  - id           : field::id
  - x, y, z      : position
  - descriptor   : compute_descriptor_mtp's B_k vector, ncoeff values
  - derivative   : compute_descriptor_mtp's compact per-atom descriptor-derivative aggregate,
                   ...
                   on them first on a multi-rank run (see compute_descriptor_mtp's doc).

No LAMMPS ground truth exists for this aggregate at any scale (LAMMPS's MTP has no compute style
at all) -- cross-validated instead via an independent Python re-implementation and finite
differences (see the plugin's test assets).

Setting `format: npy` writes a single `.npy` v1.0 file -- a float64 array of shape (natoms,
total_width), one row per atom, columns in the same order as `fields` (id: 1 column, x/y/z: 1
column each, descriptor: ncoeff columns, derivative: ncoeff*3 columns). `id` is stored as float64
in this combined array (exact for any realistic atom count).

Usage example:

compute_descriptor_mtp: { compute_derivative: true }
update_opt_from_ghost: { opt_fields: [ "mda_.*" ] }   # only needed on more than one rank
write_descriptor_mtp:
  ...
  ONIKA_AUTORUN_INIT(write_descriptor_mtp)
  {
    OperatorNodeFactory::instance()->register_factory( "write_descriptor_mtp", make_grid_variant_operator< WriteDescriptorMtp > );
  }
