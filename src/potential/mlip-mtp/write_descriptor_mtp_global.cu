...
namespace exaStamp
{
  ...
  public:
    inline void execute() override final
    {
      ...
      {
        fatal_error() << "write_descriptor_mtp_global: unknown format '"<<*format<<"' (choices: text, npy)" << std::endl;
      }

      const long nc = *ncoeff_all;
      const long rows = ( nc > 0 ) ? static_cast<long>(mtp_global->size() / static_cast<size_t>(nc)) : 0;
      ...
      if( *format == "npy" )
      {
        ...
        write_npy( onika::data_file_path(prefix+".npy"), {static_cast<size_t>(rows), static_cast<size_t>(nc)}, "<f8", mtp_global->data(), sizeof(double) );
        return;
      }
      ...
      {
        const double * const prow = mtp_global->data() + static_cast<size_t>(r)*nc;
