...
namespace exaStamp
{
  ...
  // Scratch arrays are owned by each EMTP instance and allocated at construction (Njmax-sized) --
  // the compute buffer itself stays small. Mirrors mlip-pod/include/pod_force_op.h exactly.
  struct alignas(onika::memory::DEFAULT_ALIGNMENT) MtpComputeBuffer
  {
  ...
  struct alignas(onika::memory::DEFAULT_ALIGNMENT) MtpForceOp
  {
       ...
       int type, Mat3dT& virial, CellParticlesT cells, GridCellLocksT locks, ParticleLockT& lock_a) const
    {
      ...
      EMTP& mtp = *m_mtp_interfaces[tid];

      // peratom_energyforce_soa does the full per-atom multi-pass computation (moments ->
      // contraction-DAG backprop -> per-neighbor jacobian dot) in one call, exactly mirroring
      // EAPOD::peratomenergyforce2_soa's shape -- returns per-atom energy, leaves pairwise
      // forces in mtp.soa_fij for this loop to scatter.
      const double e = mtp.peratom_energyforce_soa(
          buf.drx, buf.dry, buf.drz,
          type, buf.ext.type, jnum, m_type_map.data());

      for (int jj = 0; jj < jnum; ++jj) {
        const double fx_j = mtp.soa_fij[jj*3+0] * conv_energy_factor;
        const double fy_j = mtp.soa_fij[jj*3+1] * conv_energy_factor;
        const double fz_j = mtp.soa_fij[jj*3+2] * conv_energy_factor;
