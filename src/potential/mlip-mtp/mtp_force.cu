...
#include "include/mtp_force_op.h"
...
namespace exaStamp
{
  ...
  public:
    ...
    inline void execute() override final
    {
      ...
      // Thread contexts are pre-allocated in mtp_init. Warn if the runtime
      ...
      {
        MtpForceOp force_op{ mtp_ctx->m_emtp, mtp_ctx->type_map, !(*conv_coef_units), eflag };
  ...
  template<class GridT> using MtpForceTmpl = MtpForce<GridT>;

  ONIKA_AUTORUN_INIT(mtp_force)
  {
    OperatorNodeFactory::instance()->register_factory("mtp_force", make_grid_variant_operator<MtpForceTmpl>);
  }
