...
namespace exaStamp
{
  ...
  static inline void crg_morse(const EamCRGParameters& p, double r, phi_eval &phi)
  {
    ...
    double dk = p.gamma;
    phi.phi_pair  = p.D * ( std::exp( -2. * k) - 2. * std::exp( -k ) );
    phi.dphi_pair = -2. * dk * p.D * ( std::exp( -2. * k) - std::exp( -k ) );
  }
  ...
  {
    double pow = -r / p.rho_buck;
    double dpow = - 1. / p.rho_buck;
    double sec = - p.C / std::pow(r,6);
    double dsec = - 6. * sec / r;
    
    phi.phi_pair  = p.A * std::exp( pow ) + sec;
    phi.dphi_pair = p.A * dpow * std::exp( pow ) + dsec;
  ...
  static inline void crg_rho(const EamCRGParameters& p, double r, double& rhoValue, double& drho, bool pair_inversed = false)
  {
    ...
    double sigma=p.n/std::pow(r,8);
    double ur = 20 * ( r - 1.5);
    rhoValue  = sigma * 0.5 * ( 1 + std::erf( ur ) );
    drho = -8. * rhoValue / r + sigma * 20. * std::exp(-pow(ur,2)) / std::sqrt(M_PI);
  }
  ...
  static inline void crg_rho_mm(const EamCRGParameters& p, double r, double& rhoValue, double& drho, const EAMSpecyPairInfo& pair_info , bool pair_inversed = false )
  {
    ...
    double ur = 20 * ( r - 1.5);
    rhoValue = sigma * 0.5 * ( 1 + std::erf( ur ) );
    drho = -8. * sigma * 0.5 * ( 1 + std::erf( ur ) ) / r + 0.5 * sigma * 20. * std::exp(-pow(ur,2)) / std::sqrt(M_PI);
