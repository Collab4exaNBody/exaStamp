...
namespace exanb
{
  ...
  namespace ParticleCellProjectionTools
  {
    ...
    inline void compute_energy_gradient(
        LDBGT& ldbg
      , GridT& grid
    ...
    { 
      ...
      if( !grid_cell_values.has_field(dEdx_name) ) { grid_cell_values.add_field(dEdx_name, subdiv, 1); }
      if( !grid_cell_values.has_field(dEdy_name) ) { grid_cell_values.add_field(dEdy_name, subdiv, 1); }
      if( !grid_cell_values.has_field(dEdz_name) ) { grid_cell_values.add_field(dEdz_name, subdiv, 1); }
      ...
      ldbg << "compute_energy_gradient: subcell-centered finite differences on '" << energy_field_name
           << "' subdiv=" << subdiv << std::endl;
