...
{
  template <>
  struct ComputeCellParticlesTraits<exaStamp::CellScalarForceToParticleFunctor>
  {
...
namespace exaStamp
{
  namespace ParticleCellProjectionTools
  {
    ...
    inline void get_particle_force_from_gradient_grid(
        LDBGT &ldbg, GridT &grid, const PECFuncT &parallel_execution_context, const GridCellValues &grid_cell_values, double energy_factor)
    {
      ...
      CellScalarForceToParticleFunctor func =
          {grid.origin(), grid.offset(), grid.dimension(), subdiv, cell_size, subcell_size, inv_subcell_size, energy_factor, e_ptr, dEdx_ptr, dEdy_ptr, dEdz_ptr, e_stride, dEdx_stride, dEdy_stride, dEdz_stride};
