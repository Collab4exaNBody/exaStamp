...
namespace exaStamp
{
  ...
  static constexpr size_t AIREBO_MAX_NEIGHBORS = 512;
  static constexpr size_t AIREBO_MAX_REBO_NEIGHBORS = 32;

  struct alignas(onika::memory::DEFAULT_ALIGNMENT) AireboComputeBuffer
  {
    alignas(onika::memory::DEFAULT_ALIGNMENT) int type[AIREBO_MAX_NEIGHBORS];
  };
  ...
  ONIKA_HOST_DEVICE_FUNC ONIKA_ALWAYS_INLINE
  static double rebo_Sp(double Xij, double Xmin, double Xmax, double &dX)
  {
    double cutoff;
  ...
  ONIKA_HOST_DEVICE_FUNC ONIKA_ALWAYS_INLINE
  static double rebo_Sp2(double Xij, double Xmin, double Xmax, double &dX)
  {
    double cutoff;
  ...
  ONIKA_HOST_DEVICE_FUNC ONIKA_ALWAYS_INLINE
  static double rebo_Sp5th(double x, const double* c, double& df)
  {
    double f = c[5] * x;
    double d = 5.0 * c[5] * x;
    f += c[4];
  ...
  ONIKA_HOST_DEVICE_FUNC ONIKA_ALWAYS_INLINE
  static double rebo_Spbicubic(double x, double y,
                             const double* coeffs, double df[2])
  {
  ...
  ONIKA_HOST_DEVICE_FUNC ONIKA_ALWAYS_INLINE
  static double rebo_Sptricubic(double x, double y, double z,
                              const double* coeffs, double df[3])
  {
  ...
  ONIKA_HOST_DEVICE_FUNC ONIKA_ALWAYS_INLINE
  static double rebo_gSpline(double costh, double N, int typei,
                              const AireboParamsRO& p, double& dgdc, double& dgdN)
  {
    dgdc = dgdN = 0.0;
    if (typei == 0) {
      if (costh < p.gCdom[0]) costh = p.gCdom[0];
      if (costh > p.gCdom[4]) costh = p.gCdom[4];
      int iv = 3;
      for (int i = 0; i < 4; ++i)
        if (costh >= p.gCdom[i] && costh <= p.gCdom[i+1]) { iv = i; break; }
      double dg1, dg2;
      const double g1 = rebo_Sp5th(costh, p.gC1 + iv*6, dg1);
      const double g2 = rebo_Sp5th(costh, p.gC2 + iv*6, dg2);
      if      (N >= p.NCmax) { dgdc = dg2; return g2; }
      else if (N <= p.NCmin) { dgdc = dg1; return g1; }
      else {
        double dcut;
      ...
      if (costh < p.gHdom[0]) costh = p.gHdom[0];
      if (costh > p.gHdom[3]) costh = p.gHdom[3];
      int iv = 2;
      for (int i = 0; i < 3; ++i)
        if (costh >= p.gHdom[i] && costh <= p.gHdom[i+1]) { iv = i; break; }
  ...
  ONIKA_HOST_DEVICE_FUNC ONIKA_ALWAYS_INLINE
  static double rebo_PijSpline(double NijC, double NijH, int typei, int typej,
                                const AireboParamsRO& p, double dN2[2])
  {
    dN2[0] = dN2[1] = 0.0;
    if (typei == 1) return 0.0;
    const double* dom  = (typej == 0) ? p.pCCdom : p.pCHdom;
    const double* tbl  = (typej == 0) ? p.pCC    : p.pCH;
    if (NijC < dom[0]) NijC = dom[0];  if (NijC > dom[1]) NijC = dom[1];
    if (NijH < dom[2]) NijH = dom[2];  if (NijH > dom[3]) NijH = dom[3];
  ...
  ONIKA_HOST_DEVICE_FUNC ONIKA_ALWAYS_INLINE
  static double rebo_piRCSpline(double Nij, double Nji, double Nijconj,
                                 int typei, int typej, const AireboParamsRO& p, double dN3[3])
  {
    dN3[0] = dN3[1] = dN3[2] = 0.0;
    const double* dom;
    const double* tbl;
    if      (typei==0 && typej==0) { dom = p.piCCdom; tbl = p.piCC; }
    else if (typei==1 && typej==1) { dom = p.piHHdom; tbl = p.piHH; }
    else                           { dom = p.piCHdom; tbl = p.piCH; }
    if (Nij     < dom[0]) Nij     = dom[0];  if (Nij     > dom[1]) Nij     = dom[1];
    if (Nji     < dom[2]) Nji     = dom[2];  if (Nji     > dom[3]) Nji     = dom[3];
    if (Nijconj < dom[4]) Nijconj = dom[4];  if (Nijconj > dom[5]) Nijconj = dom[5];
  ...
  ONIKA_HOST_DEVICE_FUNC ONIKA_ALWAYS_INLINE
  static double rebo_TijSpline(double Nij, double Nji, double Nijconj,
                                const AireboParamsRO& p, double dN3[3])
  {
    dN3[0] = dN3[1] = dN3[2] = 0.0;
  ...
  struct alignas(onika::memory::DEFAULT_ALIGNMENT) REBOForceOp
  {
                           ...
                           GridCellLocksT locks, ParticleLockT& lock_a) const
    {
      ...
      // CH.airebo parameters are in LAMMPS metal units (eV, Å); convert to exaStamp internal units
      ...
      // -----------------------------------------------------------------------
      int    rebo_idx[AIREBO_MAX_REBO_NEIGHBORS];
      double rebo_r[AIREBO_MAX_REBO_NEIGHBORS];
      double rebo_w[AIREBO_MAX_REBO_NEIGHBORS];
      double rebo_dw[AIREBO_MAX_REBO_NEIGHBORS];
      int    rebo_count = 0;
      ...
      for (int aa = 0; aa < jnum; ++aa) {
        ...
        if (rsq < p.rcmaxsq[itype][atype] && r > TOL) {
          if (rebo_count >= int(AIREBO_MAX_REBO_NEIGHBORS)) {
            assert(false);
            continue;
          }
      ...
      for (int jpos = 0; jpos < rebo_count; ++jpos) {
        ...
        dVA += VA/wij * dwij;
        // Convert eV → exaStamp internal units; all force/energy terms derive from VR/VA
        VR *= conv; dVR *= conv; VA *= conv; dVA *= conv;
        ...
        for (int kpos = 0; kpos < rebo_count; ++kpos) {
          ...
          if (itype == 1) {
            ...
            fjx -= pf_lam*(-delx*inv_rij);  fjy -= pf_lam*(-dely*inv_rij);  fjz -= pf_lam*(-delz*inv_rij);
            fix -= pf_lam*((-rikx*inv_rik)+(delx*inv_rij));
            fiy -= pf_lam*((-riky*inv_rik)+(dely*inv_rij));
            fiz -= pf_lam*((-rikz*inv_rik)+(delz*inv_rij));
            fkx -= pf_lam*(rikx*inv_rik);   fky -= pf_lam*(riky*inv_rik);   fkz -= pf_lam*(rikz*inv_rik);
          }
        ...
        for (int ll = 0; ll < jnum; ++ll) {
          ...
          if (rjlsq >= p.rcmaxsq[jtype][ltype]) continue;
          ...
          const double lam = (jtype==1)
            ? 4.0*((p.rho[ltype][1]-rjl) - (p.rho[itype][1]-rij)) : 0.0;
          const double exp_lam = std::exp(lam);
        ...
        for (int ll = 0; ll < jnum; ++ll) {
          ...
          if (jtype == 1) {
            ...
            fix -= pf_lam*(delx*inv_rij);  fiy -= pf_lam*(dely*inv_rij);  fiz -= pf_lam*(delz*inv_rij);
            fjx -= pf_lam*(jlx*inv_rjl - delx*inv_rij);
            fjy -= pf_lam*(jly*inv_rjl - dely*inv_rij);
            fjz -= pf_lam*(jlz*inv_rjl - delz*inv_rij);
            flx -= pf_lam*(-jlx*inv_rjl);  fly -= pf_lam*(-jly*inv_rjl);  flz -= pf_lam*(-jlz*inv_rjl);
          }
            ...
            if (ktype == 0) {
              ...
              if (std::fabs(dNki) > TOL) {
                for (int mm = 0; mm < jnum; ++mm) {
                  ...
                  if (rkmsq >= p.rcmaxsq[ktype][mtype]) continue;
            ...
            if (ltype == 0) {
              ...
              if (std::fabs(dNlj) > TOL) {
                ...
                for (int mm = 0; mm < jnum; ++mm) {
                  ...
                  if (rlmsq >= p.rcmaxsq[ltype][mtype]) continue;
                ...
                // own buffer so must be handled separately (rln = r_l - r_i = buf.dr[ll])
                {
                  ...
                  if (rlim > TOL) {
                    ...
                    _fx -= pf2b*limx;  _fy -= pf2b*limy;  _fz -= pf2b*limz;
                  }
                }
              }
            }
        ...
        if (std::fabs(Tij) > TOL) {
          ...
          for (int kpos = 0; kpos < rebo_count; ++kpos) {
            ...
            const double rr_ik = 0.5*(rij2 + buf.d2[kk]
                                  - (r21x-r23x)*(r21x-r23x)
                                  - (r21y-r23y)*(r21y-r23y)
                                  - (r21z-r23z)*(r21z-r23z)) / (rij*r21mag);
            const double tspjik  = rebo_Sp2(rr_ik, thmin, thmax, dcut321);

            double dw21;
            const double w21 = rebo_Sp(r21mag, p.rcmin[itype][ktype],
                                        p.rcmaxp[itype][ktype], dw21);
            ...
            for (int ll = 0; ll < jnum; ++ll) {
              ...
              if (r34sq >= p.rcmaxsq[jtype][ltype]) continue;
              ...
              const double rr_jl = 0.5*(rij2 + r34mag*r34mag
                                   - (r23x+r34x)*(r23x+r34x)
                                   - (r23y+r34y)*(r23y+r34y)
                                   - (r23z+r34z)*(r23z+r34z)) / (rij*r34mag);
              const double tspijl = rebo_Sp2(rr_jl, thmin, thmax, dcut234);

              double dw34;
              const double w34 = rebo_Sp(r34mag, p.rcmin[jtype][ltype],
                                          p.rcmaxp[jtype][ltype], dw34);
              ...
              const double fcijpc = (-dt1dij*at2)
                + aaa2*(-dcut321*dctij*(1.0-tspijl) - dcut234*dctji*(1.0-tspjik));
              ...
              fi0 -= pf_w21*r21x;  fi1 -= pf_w21*r21y;  fi2 -= pf_w21*r21z;
              fk0 += pf_w21*r21x;  fk1 += pf_w21*r21y;  fk2 += pf_w21*r21z;
              const double pf_w34 = VA*Tij*sin2om*(1.0-tspjik)*(1.0-tspijl)*w21*dw34/r34mag;
              fj0 -= pf_w34*r34x;  fj1 -= pf_w34*r34y;  fj2 -= pf_w34*r34z;
              size_t cell_l, p_l;
              buf.nbh.get(ll, cell_l, p_l);
              const double hl2 = 0.5;
              _fx += hl2*fi0;  _fy += hl2*fi1;  _fz += hl2*fi2;
              ...
              fl0 += pf_w34*r34x; fl1 += pf_w34*r34y; fl2 += pf_w34*r34z; // consolidate w34 on l
        ...
        const double fpair = -(dVR + bij * dVA) / rij;
        _fx += 0.5 * delx * fpair;
        _fy += 0.5 * dely * fpair;
        _fz += 0.5 * delz * fpair;
