  ...
  {      
    ...
    inline void execute () override final
    {
      ...
      // prepare (copy) input data for worker thread
      {
	    ...
	    std::ignore = value_ptr[n-1]; // just avoid compile warnings before. remove it when you effectively use those variables
            
	    // TODO : For the simple histogram version : insitu_worker->input.insert(
	    // For the multi-analysis version, specific buffers must be sent according to the requested functionality.
            // ...
          }
        }
        insitu_worker->output.m_data.resize( *samples ); // TODO : To be modified for the multiple analysis version.
        insitu_worker->ready = true;
      }
      
      // notify worker thread data is ready
      lout << "MAIN: notify input data is ready" << std::endl;
      // TODO :  insitu_worker->cv. ...    
    }

  };

  // TODO : To be modified for the multiple analysis version.
