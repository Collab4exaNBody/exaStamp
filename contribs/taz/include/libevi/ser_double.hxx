...
namespace simd {
  ...
  /// @brief Specialization of simd::vector for SER doubles
  template <> 
  class vector<SER, double> {

  public:



    typedef vector<SER, double> self_type;
    typedef double base_t;
    typedef double chunk_t;
    ...
    self_type operator == (const self_type& a) const { return m_data == a.m_data ? self_type(0xFFFFFFFFFFFFFFFF) : s_zero; }
    self_type operator != (const self_type& a) const { return m_data != a.m_data ? self_type(0xFFFFFFFFFFFFFFFF) : s_zero; }
    self_type operator >= (const self_type& a) const { return m_data >= a.m_data ? self_type(0xFFFFFFFFFFFFFFFF) : s_zero; }
    self_type operator >  (const self_type& a) const { return m_data >  a.m_data ? self_type(0xFFFFFFFFFFFFFFFF) : s_zero; }
    self_type operator <= (const self_type& a) const { return m_data <= a.m_data ? self_type(0xFFFFFFFFFFFFFFFF) : s_zero; }
...
#define TMPL_vector vector<SER, double>
...
// No vector function.
  ...
  TMPL   inline int movemask (const TMPL_vector& a) {
    if((TMPL_vector::base_t)(a.data()) == 0xFFFFFFFFFFFFFFFF) return 1;
    else return 0;
  }
  ...
  TMPL inline TMPL_vector exp10   (const TMPL_vector& a) { 
    return TMPL_vector(::pow (10.0, a.data()));
  }
  ...
  TMPL inline TMPL_vector acos    (const TMPL_vector& a) { 
    return TMPL_vector(::acos(a.data()));
  }
